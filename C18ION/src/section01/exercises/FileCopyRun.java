...
public class FileCopyRun {
	final static String SRCFILENAME = "D:/Temp/srcfile.txt";
	final static String TARFILENAME = "D:/Temp/tarfile.txt";
	...
	public static void main(String[] args) {
		...
		filecopy.setProgress(t -> {
			switch (t.getStatus()) {
			case "오류":
				System.out.printf("복사실패 : status[%s], error[%s] %n", t.getStatus(), t.getError());
				break;

			default:
				System.out.printf("파일카피 : step[%d], status[%s], msg[%s] %n", t.getStep(), t.getStatus(), t.getMsg());
				break;
			}
		});
